...
export const metadata = { title: 'Year in Review · Semma Flow' };
...
export default async function YearInReviewPage() {
  ...
  const Stat = ({ label, value }: { label: string; value: string | number }) => (
    <div style={{ flex: 1, textAlign: 'center', padding: '14px 8px', border: '1px solid #E5E1F0', borderRadius: 14 }}>
      <div style={{ fontSize: 26, fontWeight: 800, color: '#7C3AED' }}>{value}</div>
      <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#8A8597', marginTop: 4 }}>{label}</div>
    </div>
  );

  return (
    <div style={{ background: '#fff', color: '#1A1726', minHeight: '100vh', padding: '32px 20px', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          <div>
            ...
            <p style={{ margin: '4px 0 0', fontSize: 13, color: '#8A8597' }}>
              {summary ? `${summary.from} → ${summary.to}` : 'Semma Flow'}
            </p>
          </div>
          <div className="no-print" style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
            <a
              ...
              style={{
                ...
                border: '1px solid #E5E1F0', background: '#fff', color: '#7C3AED',
        ...
        {!summary || summary.totalCompletions === 0 ? (
          <p style={{ fontSize: 15, color: '#8A8597', padding: '40px 0', textAlign: 'center' }}>
          ...
          <>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
              <thead>
                <tr style={{ textAlign: 'left', color: '#8A8597', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              ...
              <tbody>
                {[...summary.habits].sort((a, b) => b.completions - a.completions).map((h, i) => (
                  <tr key={i} style={{ borderTop: '1px solid #EEEAF6' }}>
                    <td style={{ padding: '10px 6px', fontWeight: 600 }}>
                      {h.name} {h.isBad && <span style={{ color: '#8A8597', fontWeight: 400 }}>(avoid)</span>}
                    </td>
            ...
            <p style={{ marginTop: 28, fontSize: 11, color: '#B0AABF', textAlign: 'center' }}>
              Generated by Semma Flow · Use “Save as PDF” or your browser’s print dialog.
            </p>
          </>
        )}
      </div>
